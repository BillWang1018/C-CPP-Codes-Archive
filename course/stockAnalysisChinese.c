...
void findMinMaxTotalShareholders(SD* data, int N) {
    ...
    printf("對應最低收盤價: %.2lf 人數: %d\n", data[mi_idx].closingPrice, data[mi_idx].totalHolder);
    printf("對應最高收盤價: %.2lf 人數: %d\n", data[ma_idx].closingPrice, data[ma_idx].totalHolder);
}
...
void calculateChanges(SD *data)
{
    ...
    printf("最近一週大股東進出比率: %.2lf\n", ratio_last1);
    printf("最近四週大股東進出比率: %.2lf\n", ratio_last4);
}
